...
wx.MessageBox(' -_-  Add Extension and Then Click On OK BUTTON -_- ', 'CA Special Translation Exe', wx.OK | wx.ICON_ERROR)
time.sleep(5)
try:
...
query = open("C:\\Translation EXE\\CA_query.txt", "r")
...
def tarnslation():
    try:
        ...
        cur.execute(str(Text_query))  # 0 = English, 1 = Non-English
        rows = cur.fetchall()
                ...
                if not re.match("^[\W A-Za-z0-9_@?./#&+-]*$", notice_no):
                    ...
                    for input_xpath in input_xpath_list:
                        ...
                        if is_available == 0:
                            break
                    if is_available == 1:
                        wx.MessageBox('Something Went Wrong Please Refresh Google Translation Page Then Click On -_- OK -_- ','Contract Award Special Google Translation Exe ', wx.OK | wx.ICON_WARNING)
                        time.sleep(2)
